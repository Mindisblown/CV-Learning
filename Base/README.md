...
​		图像白化（whitening）可用于对过度曝光或低曝光的图片进行处理，处理的方式就是改变图像的平均像素值为 0 ，改变图像的方差为单位方差 1。先计算原图像的均值和方差，然后对原图像的每个像素值做变换(减均值除方差)。

# DiceLoss

​		相似度度量函数，通常用于计算两个样本的相似度。|X∩Y|表示pred与gt的点乘，并且将点乘的元素相加，|X|的量化可直接求和等方法。
$$
